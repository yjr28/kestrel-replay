...
Retry semantics are not modeled by the current span localizers. For single-run comparisons, more than one identified application span with the same `(service, operation)` makes that key ambiguous, so ranked localization and the earlier single-divergence primitive abstain on that key rather than choosing an event by timestamp or input order. Profile-vs-failing localization applies the same rule to duplicate keys in the failing run, preventing ambiguity from being mislabeled as a missing span. Reusing the same identified span event ID within a run is also ambiguous provenance: every affected localization key is marked ambiguous, so single-run and profile-vs-failing localization abstain on those keys rather than turning a provenance failure into `missing_span` or `unexpected_span` evidence. Healthy-profile construction rejects a healthy run that reuses an eligible application-span event ID rather than choosing one observation. Healthy-profile construction likewise rejects duplicate eligible spans for the same `(service, operation)` key. Spans without event identity or without a nameable service/operation key remain ineligible evidence and therefore do not create either ambiguity.
