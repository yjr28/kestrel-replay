...
- application span status changes when both compared statuses are non-blank;
...
Application-span evidence must have a non-blank event ID and a non-blank `(service, operation)` localization key before it can establish or satisfy a divergence. Surrounding whitespace is trimmed from service and operation before localization keys are compared or stored, including in multi-run healthy profiles, so formatting-only whitespace cannot split one logical span key into missing/unexpected evidence. This eligibility rule applies to ranked single-run localization, the earlier single-divergence primitive, and multi-run healthy span profiles. It keeps provenance auditable and the localization target nameable: an unidentified or unkeyed span cannot silently replace a missing healthy/failing observation, create an `unexpected_span` candidate, or mark a healthy topology key as observed. Single-run divergences carry the exact healthy/failing event IDs that support the comparison when those events exist. Multi-run profile baselines retain the eligible event ID from every healthy run contributing to a stable baseline and also retain one representative healthy event ID for current profile-vs-failing ranking output. Profile-vs-failing candidates therefore still expose the representative healthy ID plus the exact failing event ID; the complete healthy provenance remains available from the profile baseline rather than being implied by that representative candidate field. When the external terminal service affects ranking, the candidate records an `outcome.terminal_service=<service>` anchor.

Status is optional span metadata rather than span identity. A `status_change` or `terminal_status_change` candidate therefore requires non-blank status on both observations being compared. Surrounding whitespace is trimmed before status equality is evaluated, so formatting-only whitespace differences do not establish status-change evidence; emitted status-change values and stable healthy-profile status baselines use that normalized value. For multi-run profiles, status is considered stable only when every healthy run supplies the same normalized non-blank value; a blank healthy status makes status evidence unavailable for that baseline without removing the span from topology or duration evidence.

Retry semantics are not modeled by the current span localizers. For single-run comparisons, more than one identified application span with the same `(service, operation)` makes that key ambiguous, so ranked localization and the earlier single-divergence primitive abstain on that key rather than choosing an event by timestamp or input order. Profile-vs-failing localization applies the same rule to duplicate keys in the failing run, preventing ambiguity from being mislabeled as a missing span. Reusing the same non-blank event ID across application spans within a run is also ambiguous provenance: every otherwise eligible localization key carried by those spans is marked ambiguous, even when another span sharing the ID has an unusable service/operation key. Single-run and profile-vs-failing localization therefore abstain on those eligible keys rather than treating a non-unique event ID as exact provenance or turning that provenance failure into `missing_span` or `unexpected_span` evidence. Healthy-profile construction rejects a healthy run that reuses an eligible application-span event ID rather than choosing one observation. Healthy-profile construction likewise rejects duplicate eligible spans for the same `(service, operation)` key. Spans without event identity or without a nameable service/operation key remain ineligible to establish localization evidence; an unkeyed span can still invalidate another span's provenance when both reuse the same non-blank event ID.

The corpus currently builds its healthy span profile from three separately recorded healthy executions. The profile stores descriptive timing/count statistics; these are empirical regression baselines, not calibrated probability models. Profile latency ranking uses the configured minimum threshold or six times the healthy median absolute deviation, whichever is larger. If that MAD scaling cannot be represented safely as a `time.Duration`, latency evidence for that baseline is treated as unavailable rather than allowing arithmetic overflow to lower the threshold.
...
Kestrel also builds an empirical message-topology profile from the same healthy-run set. Only application message events with a non-blank event ID, non-blank `topic`, supported `message.action`, non-blank service, and non-blank `message.id` are eligible to affect this profile or a failing-run topology comparison. Surrounding whitespace is trimmed from topic, action, service, and message ID for eligibility, and the normalized `(topic, action, service)` tuple is used as the observable flow key. Formatting-only whitespace therefore cannot split one logical flow into missing and unexpected topology evidence. The event ID is required because topology divergences expose exact event provenance; an event that cannot be named cannot establish or satisfy that evidence. The message ID remains a separate eligibility requirement for message correlation metadata and is not part of the current count-envelope key.

Reusing the same non-blank event ID across application message events makes the multiplicity of every otherwise keyable affected flow ambiguous, including when another message sharing that ID has an unusable flow key. Failing-run comparison abstains on an affected flow rather than dropping the reused events and reinterpreting their absence as `count_below_healthy_range` or `unexpected_message_flow` evidence. Healthy-profile construction likewise withholds a flow from its empirical count envelope if that flow is ambiguous in any healthy run. The profile still remembers that the flow was observed in healthy evidence, so a later valid failing observation is not relabeled as a never-seen unexpected flow. This is provenance abstention, not retry or duplicate-delivery semantics.

For each unambiguous eligible `(topic, action, service)` flow the profile records the observed count envelope plus per-run evidence: the count and exact application event IDs observed in each healthy run. A genuinely absent flow in a particular healthy run is represented explicitly as a zero count with no event IDs; an ambiguous flow is not converted into such a zero-count observation.

`graph.CompareMessageTopology` reports only eligible, unambiguous application message evidence. For flows that exceed or fall below a healthy envelope, the divergence carries the per-run healthy evidence and the exact failing event IDs. For a flow never seen in the healthy profile, the divergence carries an explicit zero-count record for every healthy run together with the failing events that establish the newly observed flow. It can distinguish:
