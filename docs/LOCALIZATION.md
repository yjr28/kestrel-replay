...
Status is optional span metadata rather than span identity. A `status_change` or `terminal_status_change` candidate therefore requires non-blank status on both observations being compared. For multi-run profiles, status is considered stable only when every healthy run supplies the same non-blank value; a blank healthy status makes status evidence unavailable for that baseline without removing the span from topology or duration evidence.
