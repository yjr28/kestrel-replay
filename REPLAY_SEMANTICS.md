...
Structurally accepted message evidence must be application-originated and carry nonblank trace, service, message ID, publish/consume action, and topic identity. This keeps evidence that cannot participate in the implemented message topology or replay correlation from entering the collector as valid message evidence.

Generated message/span IDs and timestamps are deliberately excluded from the signature so a fresh replay process can be compared semantically, but they still gate correlation internally. A consume contributes to delivery parity only when its nonblank `trace_id` and `message.id` identify exactly one observed publish in the same trace that can be established as preceding that consume. Reuse of the same message ID in another trace neither satisfies nor makes that correlation ambiguous. A later same-trace reuse of the same message ID does not retroactively erase already-established correlation, while multiple preceding same-trace publishes remain ambiguous and are withheld from the canonical delivery counts. Distinct timestamps establish temporal precedence directly. Equal timestamps require complete, nonzero sequence evidence on both events; missing or partial sequence evidence is withheld rather than interpreted as an ordering relation.

## Async message-delay evidence

For the implemented delayed-message slice, matching HTTP outcome and delivery multiplicity are still insufficient. Kestrel correlates application publish and consume evidence by the pair (`trace_id`, `message.id`), then derives the minimum observed publish-to-consume delay for the target topic. Both the recorded incident and the fresh replay must meet the configured positive delay threshold.

Delay comparison is deliberately threshold-based rather than exact wall-clock equality. The canonical delay signature records microseconds; a positive configured threshold below one microsecond is therefore not representable by this evidence and is withheld rather than rounded down into a satisfied zero-microsecond threshold. Correlation is withheld when a consume predates its publish, when there are multiple same-trace publishes that can be established as preceding the consume for the same message ID, or when equal-timestamp ordering lacks complete nonzero sequence evidence. A later same-trace publish does not invalidate an otherwise unambiguous earlier publish-to-consume correlation, and another trace's reuse of the same message ID is outside that correlation. This is evidence gating for the tested broker-delay slice, not a claim of deterministic queue timing or controlled message ordering.
