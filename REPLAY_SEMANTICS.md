...
Generated message/span IDs and timestamps are deliberately excluded from the signature so a fresh replay process can be compared semantically, but they still gate correlation internally. A consume contributes to delivery parity only when its nonblank `message.id` has exactly one observed publish that can be established as preceding that consume. A later reuse of the same message ID does not retroactively erase already-established correlation, while multiple preceding publishes remain ambiguous and are withheld from the canonical delivery counts. Distinct timestamps establish temporal precedence directly. Equal timestamps require complete, nonzero sequence evidence on both events; missing or partial sequence evidence is withheld rather than interpreted as an ordering relation.
...
Delay comparison is deliberately threshold-based rather than exact wall-clock equality. Correlation is withheld when a consume predates its publish, when there are multiple publishes that can be established as preceding the consume for the same message ID, or when equal-timestamp ordering lacks complete nonzero sequence evidence. A later same-ID publish does not invalidate an otherwise unambiguous earlier publish-to-consume correlation. This is evidence gating for the tested broker-delay slice, not a claim of deterministic queue timing or controlled message ordering.
