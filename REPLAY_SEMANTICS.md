...
Generated message/span IDs and timestamps are deliberately excluded from the signature so a fresh replay process can be compared semantically, but they still gate correlation internally. A consume contributes to delivery parity only when its nonblank `message.id` has exactly one observed publish that can be established as preceding that consume. A later reuse of the same message ID does not retroactively erase already-established correlation, while multiple preceding publishes remain ambiguous and are withheld from the canonical delivery counts. Equal-timestamp ordering uses sequence evidence when it is available; incomplete sequence evidence is not used to establish a partial ordering.
