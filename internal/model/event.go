...
func (e Event) Validate() error {
	...
	case KindFault:
		...
		case "service_crash":
			if targetOperation != "" {
				return errors.New("service_crash fault events are process-scoped and do not accept target.operation")
			}
		default:
...
func (e Event) CanonicalKey() string {
	fields := []string{
		string(e.Source),
		string(e.Kind),
		strings.TrimSpace(e.Service),
		strings.TrimSpace(e.Operation),
		strings.TrimSpace(e.Status),
	}
	var key strings.Builder
	for _, field := range fields {
		_, _ = fmt.Fprintf(&key, "%d:%s", len(field), field)
	}
	return key.String()
}
