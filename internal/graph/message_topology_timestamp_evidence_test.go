...
func TestMessageTopologyAbstainsOnUntimestampedFailingFlowEvidence(t *testing.T) {
	now := time.Now().UTC()
	...
	for i := range failing {
		if failing[i].Service == "analytics" && failing[i].Attributes["message.action"] == "consume" {
			failing[i].Timestamp = time.Time{}
			break
		}
	}

	if divergences := CompareMessageTopology(profile, failing); len(divergences) != 0 {
		t.Fatalf("untimestamped flow evidence must make the key uncertain rather than prove presence or absence: %#v", divergences)
	}
}

func TestMessageTopologyWithholdsBaselineWhenHealthyFlowEvidenceIsUntimestamped(t *testing.T) {
	now := time.Now().UTC()
	run1 := messageRun("h1", now, 1)
	for i := range run1 {
		if run1[i].Service == "analytics" && run1[i].Attributes["message.action"] == "consume" {
			run1[i].Timestamp = time.Time{}
			break
		}
	}
	...
	for _, baseline := range profile.Baselines() {
		if baseline.Service == "analytics" && baseline.Action == "consume" && baseline.Topic == "orders.completed" {
			t.Fatalf("healthy baseline must be withheld when any run has untimestamped evidence for the flow: %+v", baseline)
		}
	}
}
